...
- 电影：依次搜索“中文标题 + 年份”“原始标题 + 年份”“中文标题”“原始标题”，
  找到有效候选后停止回退。
- 电影标题明确匹配时，允许作品页年份与媒体库年份相差 1 年；更大的年份差仍会
  被候选筛选拒绝。
- 剧集：整季包优先。中文标题先使用“剧名 + 第二季”，原始标题使用
  “剧名 + Season 2”，再回退到 `S02`。整季无结果时，按中文“第二季 + 第三集”、
  英文“Season 2 + Episode 3”和 `S02E03` 的顺序精确搜索单集。
- 剧集年份同时接受父级剧集首播年与当季/单集首播年，兼容字幕站对续季发行年份的
  不同标注。
- 仅返回 SRT、ASS、SSA、VTT 文本字幕。
- 识别简体、繁体和站点仅标记为“中英双语/双语”的中文字幕；纯外语字幕不返回。
- 通过身份和语言校验后，优先尝试下载量更高的候选，字幕库评分作为次要站内信号；
  只有质量接近时才优先双语字幕。
...
- 下载量和评分来自站点页面，仅作为同一作品内的排序信号，不代表字幕一定适配当前
  视频发行版本。
